...
//----------------------------------------------------------------------------------
func NewOrdererYaml(filePath string) (*OrdererYaml, error) {
	config := &OrdererYaml{}
	...
	if err != nil {
		return nil, fmt.Errorf("error unmarshalling OrdererYaml: %v", err)
	}
...
//----------------------------------------------------------------------------------
func (c *OrdererYaml) Write(filePath string, perm os.FileMode) error {
...
func (c *OrdererYaml) Merge(config *OrdererYaml) {
	util.MergeStructs(c, config, "Merge OrdererYaml configs")
}
